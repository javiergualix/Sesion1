package ejercicio2;

import java.util.Date;

public class CuantosPrimos2 {
    public static void main(String[] x) {
        long ti = (new Date()).getTime(); //t0=instante de inicio de los cálculos
        PrimosThread p1 = new PrimosThread(1,2000000);
        PrimosThread p2 = new PrimosThread(2000000,4000000);
        PrimosThread p3 = new PrimosThread(4000000,6000000);
        PrimosThread p4 = new PrimosThread(6000000,8000000);
        PrimosThread p5 = new PrimosThread(8000000,10000000);
        long t0 = (new Date()).getTime();
        p1.start();
        long t1 = (new Date()).getTime();
        p2.start();
        long t2 = (new Date()).getTime();
        p3.start();
        long t3 = (new Date()).getTime();
        p4.start();
        long t4 = (new Date()).getTime();
        p5.start();
        try {
            p1.join();p2.join();;p3.join();p4.join();p5.join(); //esperamos a que terminen todos

        } catch (InterruptedException e) {}
        long t6 = (new Date()).getTime();
        int n = p1.cuantos() + p2.cuantos() + p3.cuantos() + p4.cuantos() + p5.cuantos();
        long tf = (new Date()).getTime(); //t1 = instante de final de los cálculos
        System.out.println("Tramo 1, calculado en " + (t6-t0));
        System.out.println("Tramo 2, calculado en " + (t6-t1));
        System.out.println("Tramo 3, calculado en " + (t6-t2));
        System.out.println("Tramo 4, calculado en " + (t6-t3));
        System.out.println("Tramo 5, calculado en " + (t6-t4));
        System.out.printf("Número de primos menores que 10.000.000: " + n + " calculado en " + (tf-ti) + " miliseg.");
    }
}
